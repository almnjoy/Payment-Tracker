...
// Exchange an active browser session for mobile JWT tokens.
router.post("/auth/token", tokenMintRateLimiter, isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    if (!profile?.role) return res.status(403).json({ message: "User has no assigned role" });
...
// Mint a new access token using a valid refresh token (no cookie required).
router.post("/auth/refresh", refreshRateLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!profile?.role) return res.status(403).json({ message: "User profile no longer valid" });
...
// Revoke refresh token and/or blacklist the current access token JTI.
...
// GET /api/mobile/me
// Return user profile for a Bearer-authenticated request.
